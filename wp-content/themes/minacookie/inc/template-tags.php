...
function twentyfifteen_comment_nav() {
	...
	<nav class="navigation comment-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Comment navigation', 'wp-mango' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( __( 'Older Comments', 'wp-mango' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( __( 'Newer Comments', 'wp-mango' ) ) ) :
...
if ( ! function_exists( 'wp_senturia_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 *
 * @since Twenty Fifteen 1.0
 */
function wp_senturia_entry_meta() {
	if ( is_sticky() && is_home() && ! is_paged() ) {
		printf( '<span class="sticky-post">%s</span>', __( 'Featured', 'wp-mango' ) );
	}
	...
	if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
		...
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<i class="fa fa-calendar"></i><time class="entry-date published link" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
	...
	if ( 'post' == get_post_type() ) {
		...
		$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'wp-mango' ) );
		if ( $categories_list && twentyfifteen_categorized_blog() ) {
			printf( '<span class="cat-links tags"><span class="screen-reader-text">%1$s </span>%2$s</span>',
				_x( 'Categories', 'Used before category names.', 'wp-mango' ),
				$categories_list
			);
		}
...
function twentyfifteen_post_thumbnail() {
	...
	<a class="post-thumbnail tRes tRes_75" href="<?php the_permalink(); ?>" aria-hidden="true">
		<?php
...
function twentyfifteen_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		...
		sprintf( __( 'Continue reading %s', 'wp-mango' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
